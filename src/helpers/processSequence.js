...
const isValidLength = compose(
  allPass([isLessThanTen, isGreaterThanTwo]),
  length
);
const isValidInput = compose(
  allPass([isValidLength, isPositiveNumber, isFloatNumber]),
  getValue
);
...
const handleValidationError = (input) =>
  getHandleError(input)("ValidationError");
